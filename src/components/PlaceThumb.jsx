...
// Import all images from src/assets at build time
const images = import.meta.glob("../assets/*.{png,jpg,jpeg,PNG,JPG,JPEG}", {
  eager: true,
  import: "default",
});
...
function resolveImage(name) {
  ...
  const key = Object.keys(images).find((path) => {
    ...
    return (
      lowerPath.endsWith(`/${filename}.png`) ||
      lowerPath.endsWith(`/${filename}.jpg`) ||
      lowerPath.endsWith(`/${filename}.jpeg`)
    );
  });
...
export default function PlaceThumb({ name }) {
  ...
  function handleEnter() {
    ...
    left = Math.max(
      8,
      Math.min(left, window.innerWidth - PREVIEW_W - 8)
    );
  ...
  if (!src) {
    return (
      <div className="w-40 h-24 rounded-md bg-paperDeep flex items-center justify-center text-center px-1.5 shrink-0">
        <span className="text-[9px] font-mono text-ink/50 leading-tight">
          {name}
        </span>
      </div>
    );
